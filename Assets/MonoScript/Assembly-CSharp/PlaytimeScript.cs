...
public class PlaytimeScript : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		if (!ps.jumpRope)
		{
			...
			RaycastHit hitInfo;
			if (Physics.Raycast(base.transform.position, direction, out hitInfo, float.PositiveInfinity, 769, QueryTriggerInteraction.Ignore) & (hitInfo.transform.tag == "Player") & ((base.transform.position - player.position).magnitude <= 80f) & (playCool <= 0f))
			{
				playerSeen = true;
				TargetPlayer();
			}
